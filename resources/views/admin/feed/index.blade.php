...
@section('content')
    <div class="mb-8 flex flex-wrap items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold tracking-tight">Feed Instagram</h1>
            <p class="mt-1 text-sm text-neutral-500">{{ $posts->total() }} post — tampil di beranda</p>
        </div>
        <div class="rounded-2xl border border-neutral-200 bg-white p-3">
            ...
            <p class="mt-2 text-[11px] text-neutral-400">File hasil <code>python/scraper/instagram_feed.py</code> (data/instagram.json)</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-5">
        @forelse($posts as $post)
            <div class="rounded-2xl border border-neutral-200 bg-white p-3">
                ...
                <p class="mt-2 line-clamp-2 text-xs text-neutral-500">{{ $post->caption }}</p>
                <p class="mt-1 text-[10px] text-neutral-400">{{ $post->posted_at?->diffForHumans() ?? 'Tanpa tanggal' }}</p>
                <div class="mt-2 flex gap-2">
                    <a href="{{ $post->permalink }}" target="_blank" rel="noopener" class="flex-1 rounded-lg border border-neutral-200 py-1.5 text-center text-xs transition hover:bg-neutral-100">Buka</a>
        ...
        @empty
            <div class="col-span-full rounded-2xl border border-neutral-200 bg-white p-16 text-center text-sm text-neutral-400">

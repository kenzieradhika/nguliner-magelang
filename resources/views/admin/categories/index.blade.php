...
@section('content')
    <div class="mb-8">
        <h1 class="text-2xl font-bold tracking-tight">Kategori Kuliner</h1>
        <p class="mt-1 text-sm text-neutral-500">{{ $categories->total() }} kategori</p>
    </div>
    ...
    <div class="grid gap-6 lg:grid-cols-3">
        ...
        <div class="overflow-x-auto rounded-2xl border border-neutral-200 bg-white lg:col-span-2">
                ...
                <tbody class="divide-y divide-neutral-100">
                    @foreach($categories as $category)
                        <tr>
                            ...
                            <td class="px-5 py-4 text-neutral-400">{{ $category->slug }}</td>
                            <td class="px-5 py-4">{{ $category->places_count }} tempat</td>
                            <td class="px-5 py-4">
                                <div class="flex justify-end gap-2">
                                    <button type="submit" class="rounded-lg border border-neutral-200 px-3 py-1.5 text-xs transition hover:bg-neutral-100">Simpan</button>
                                </form>
            ...
            <div class="border-t border-neutral-100 p-4">{{ $categories->links() }}</div>
        </div>
    </div>
@endsection
